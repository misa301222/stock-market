import { faHandHoldingDollar, faSearch } from "@fortawesome/free-solid-svg-icons";
...
function NewTradeStock() {
    const [search, setSearch] = useState<string>();
    const [userProfiles, setUserProfiles] = useState<UserProfile[]>();
    const [notFound, setNotFound] = useState<boolean>(false);
    const navigate = useNavigate();

    const handleOnClickTradeStock = (email: string) => {
        navigate(`tradeStockInfo/${email}`);
    }
    ...
    return (
        <div>
            <div className="container mx-auto">
                <h1 className="header mt-10">New Trade <FontAwesomeIcon icon={faHandHoldingDollar} /></h1>
                <hr />
            </div>
                    ...
                    !notFound ?
                        userProfiles?.map((element: UserProfile, index: number) => (
                            <motion.div className="card w-96 cursor-pointer h-96" key={index}
                                ...
                                onClick={() => handleOnClickTradeStock(element.email)}>
                                <div className="">
                                    <div
                                        style={{
